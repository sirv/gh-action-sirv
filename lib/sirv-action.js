function createAction(deps) {
  ...
  function listTopLevelEntries(manifest) {
    const seen = new Set();
    const entries = [];

    for (const file of manifest) {
      const [topLevelName] = file.relativePath.split('/');
      if (!topLevelName || seen.has(topLevelName)) {
        continue;
      }

      seen.add(topLevelName);
      entries.push(topLevelName);
    }
  ...
  async function deployStaged(manifest, liveDir) {
    ...
    if (liveDir === '/') {
      await deployStagedToRoot(stagedManifest, releaseDir, backupDir, backupsRoot);
      setCompatibilityOutputs(liveDir);
      return;
    }
  ...
  async function deployStagedToRoot(stagedManifest, releaseDir, backupDir, backupsRoot) {
    const topLevelEntries = listTopLevelEntries(stagedManifest);
    const reservedNames = new Set(['.__releases', '.__backups']);

    for (const name of topLevelEntries) {
      if (reservedNames.has(name)) {
        throw new Error(`Cannot deploy root entry "${name}" because it conflicts with staged deploy metadata paths.`);
      }
    }
    ...
    for (const name of topLevelEntries) {
      const livePath = joinRemotePath('/', name);
      const backupPath = joinRemotePath(backupDir, name);

      if (await pathExists(livePath)) {
        await withRetry(() => renamePath(livePath, backupPath), `RENAME ${livePath} -> ${backupPath}`);
        backedUpEntries.push(name);
      }
    }

    core.setOutput('backup_path', backupDir);
    core.info(`Backed up ${backedUpEntries.length} root entries to ${backupDir}`);

    try {
      for (const name of topLevelEntries) {
        const stagedPath = joinRemotePath(releaseDir, name);
        const livePath = joinRemotePath('/', name);
        await withRetry(() => renamePath(stagedPath, livePath), `RENAME ${stagedPath} -> ${livePath}`);
        promotedEntries.push(name);
      }
    } catch (error) {
      if (rollbackOnFailure) {
        core.warning(`Root cutover failed; attempting to restore ${backupDir}`);
        try {
          for (const name of promotedEntries.slice().reverse()) {
            const livePath = joinRemotePath('/', name);
            const stagedPath = joinRemotePath(releaseDir, name);
            if (await pathExists(livePath)) {
              await withRetry(() => renamePath(livePath, stagedPath), `ROLLBACK ${livePath} -> ${stagedPath}`);
            }
          }

          for (const name of backedUpEntries.slice().reverse()) {
            const backupPath = joinRemotePath(backupDir, name);
            const livePath = joinRemotePath('/', name);
            if (await pathExists(backupPath)) {
              await withRetry(() => renamePath(backupPath, livePath), `ROLLBACK ${backupPath} -> ${livePath}`);
            }
          }
        } catch (rollbackError) {
    ...
    core.info(`Promoted ${topLevelEntries.length} root entries from ${releaseDir} to /`);
  }
